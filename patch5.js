...
const blockRegex = /\{editData\.wilaya === 'غرداية' && \([\s\S]*?<\/select>\s*\}\)\s*<input\s*type="text"\s*value=\{editData\.address \|\| ''\}\s*onChange=\{[\s\S]*?className="w-full bg-\[\#0f0a08\] border border-brand-gold\/30 rounded-lg p-2 text-brand-cream"\s*\/>/;
...
content = content.replace(blockRegex, newChunk);
